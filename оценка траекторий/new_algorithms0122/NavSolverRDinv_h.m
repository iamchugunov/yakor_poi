function [X, flag, dop, nev, hei_geo] = NavSolverRDinv_h(y, posts, X0, h, config)
    min_i = 0;
    min_nev = 1e20;
    min_hei_geo = 0;
    for i = 1:length(h)
        [X(:,i), dop(i), nev(i), flag] = NavSolverRDinv(y, config.posts, [0;0], h(i));
