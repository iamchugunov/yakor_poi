function [f] = dpdxdy(y,x,t,i,j, config)
    ...
    f1 = - 1/Sn^2 * y * 1/config.c * (x_t(x,t) - X(1,i))*(y_t(x,t) - X(2,i))/R_t(x, t, X(:,i))^3/2;
    ...
    f2 = 1/Sn^2 * 1/config.c * ( 1/config.c * (y_t(x,t) - X(2,i))/R_t(x, t, X(:,i)) * (x_t(x,t) - X(1,i))/R_t(x, t, X(:,i)) + S_t(x, t, i, j, config) * f3);
    f = f1 - f2;
end
